...
CREATE TABLE WATER_INFO (
    ...
    PROPERTIES VARCHAR2(255) NOT NULL,
    RECOMMENDATIONS VARCHAR2(255) NOT NULL,
    ...
    CONSTRAINT CHK_PROPERTIES_FORMAT
        CHECK (REGEXP_LIKE(PROPERTIES, '^[A-Za-zА-Яа-я0-9 ,.-]+$')),

    CONSTRAINT CHK_RECOMMENDATIONS_FORMAT
        CHECK (REGEXP_LIKE(RECOMMENDATIONS, '^[A-Za-zА-Яа-я0-9 ,.-]+$'))
);
...
CREATE TABLE SUPPORT_TOOL (
    ...
    TOOL_TYPE VARCHAR2(30) NOT NULL,

    CONSTRAINT CHK_TOOL_NAME
        CHECK (REGEXP_LIKE(TOOL_NAME, '^[A-Za-zА-Яа-я0-9 ]+$'))
);
...
CREATE TABLE RESULT (
    ...
    CONSTRAINT CHK_RESULT_TYPE
        CHECK (RESULT_TYPE IN ('audio', 'text', 'exercise'))
);
